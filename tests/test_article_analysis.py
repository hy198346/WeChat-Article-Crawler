...
class TestCrawlerSingleAnalysisIntegration(unittest.TestCase):
    ...
    def test_run_async_job_file_external_failure_notifies_and_keeps_terminal_job_record(self):
        notifications = []
        spawned = []
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as d:
                ...
                self.assertTrue(job_path.exists())
                stopped_job = json.loads(job_path.read_text(encoding="utf-8"))
                self.assertEqual(stopped_job.get("status"), "failed_external")
                retry_state = stopped_job.get("retry_state") or {}
                self.assertEqual(retry_state.get("stop_reason"), "wechat_auth_required")
                self.assertEqual(retry_state.get("last_reason"), "wechat_auth_required")
                self.assertEqual(spawned, [])
        ...
        try:
            with tempfile.TemporaryDirectory() as d:
                article = {
                    ...
                    "published_at": "2026-06-11 21:30",
                    "url": "https://mp.weixin.qq.com/s/a?__biz=MjM5&mid=100&idx=1&sn=abc",
                    "_raw_article": {"title": "A 文", "link": "https://mp.weixin.qq.com/s/a?__biz=MjM5&mid=100&idx=1&sn=abc"},
                }
                expected_article_id = article_analysis.build_article_id(article)
                wechat_crawler.load_accounts_list = lambda config, accounts_file_override=None: [
                    {"name": "号A", "fakeid": "fidA", "group": "测试分组"}
                ]
                wechat_crawler._extract_latest_payload_for_account = lambda **kwargs: dict(article)
                wechat_crawler.fetch_article_markdown = lambda article, headers, account_name=None: {
                    "account": "号A",
                    "title": "A 文",
                    "date": "2026-06-11",
                ...
                wechat_crawler.push_articles_to_serverchan = (
                    lambda config, articles, override_sendkey=None, batch_analysis=None: captured.update(
                        {"articles": articles, "batch_analysis": batch_analysis}
                    )
                    or {"ok": True}
                )
                wechat_crawler.analyze_single_article = (
                    lambda config, article: {"status": "ok", "article_id": "aid1", "summary": "单篇解读"}
                )
                wechat_crawler.summarize_analysis_batch = (
                    lambda config, analyses, batch_id: {
                        "status": "ok",
                        "batch_id": batch_id,
                        "batch_focus": "情绪修复",
                        "shared_themes": ["资金回流"],
                ...
                wechat_crawler.build_analysis_index_html = lambda config: None
                wechat_crawler._schedule_async_job = (
                    lambda name, func, *args, **kwargs: {"status": "scheduled", "name": name}
                )
                ...
                self.assertEqual(len(captured["articles"]), 1)
                self.assertEqual(captured["articles"][0]["article_id"], expected_article_id)
                self.assertEqual(
                    article_analysis.build_article_id(captured["articles"][0]),
                    expected_article_id,
                )
        finally:
            wechat_crawler.load_accounts_list = old_load_accounts
...
class TestBatchSummaryOutput(unittest.TestCase):
    ...
    def test_run_batch_analysis_pipeline_converges_with_terminal_single_article_failure(self):
        captured = {"attach_calls": 0, "batch_items": None}
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as d:
                wechat_crawler.OUTPUT_ROOT = Path(d)
                jobs_dir = wechat_crawler.OUTPUT_ROOT / "async_jobs"
                jobs_dir.mkdir(parents=True, exist_ok=True)
                (jobs_dir / "single-failed.json").write_text(
                    json.dumps(
                        {
                            "name": "single_article_analysis",
                            "job_type": "single_article_analysis",
                            "article_id": "aid-terminal",
                ...
                self.assertEqual(result["status"], "ok")
                self.assertEqual(captured["attach_calls"], 0)
                self.assertEqual(captured["batch_items"][0]["status"], "skipped")
                self.assertEqual(captured["batch_items"][0]["summary"], "")
        finally:
            wechat_crawler.OUTPUT_ROOT = old_output_root
            if old_attach is not None:
                wechat_crawler._attach_single_article_analysis = old_attach
    ...
    def test_drain_analysis_queue_skips_batch_jobs_for_recovery(self):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as d:
                ...
                jobs_dir = wechat_crawler.OUTPUT_ROOT / "async_jobs"
                jobs_dir.mkdir(parents=True, exist_ok=True)
                batch_job = jobs_dir / "batch-retry.json"
                batch_job.write_text(
                    json.dumps(
                        {
                            "name": "push_latest_all_analysis",
                            "job_type": "batch_analysis_pipeline",
                            ...
                            "retry_state": {
                                "attempt": 2,
                                "retry_mode": "until_success",
                                "last_reason": "waiting_single_article_queue",
