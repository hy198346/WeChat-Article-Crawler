...
run_drain() {
  local flag="$1"
  echo "[$(timestamp)] run $flag"
  /usr/bin/python3 "$ROOT/scripts/wechat_article_crawler/wechat_crawler.py" "$flag"
}

mkdir -p "$ROOT/logs" "$ROOT/output"
cd "$ROOT"

run_drain --drain-analysis-queue
run_drain --drain-batch-followup-queue
